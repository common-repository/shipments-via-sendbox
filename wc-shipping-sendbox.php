...
if (!class_exists('WC_Shipping_Sendbox')) {
	...
	$this->instance_id    = absint( $instance_id );
        ...
        if($this->get_option( 'test_mode' )=="yes")
	     ...
	     $display_frontend=$this->get_option( 'display_frontend' );
       ...
       $this->get_shipping_options();
           ...
           function calculate_shipping($package = array()) {
			   ...
			   if($this->get_option( 'display_frontend' )=="yes")
			   {
...
'options'    => $countries,
	...
	'display_frontend' => array(
        'title'         => __( 'Display in Frontend', 'woocommerce' ),
        'type'          => 'select',
        'class'         => 'wc-enhanced-select',
        'default'       => 'yes',
        'options'       => array(
	...
	'auth_header' => array(
        'title'         => __( 'Authorization Header', 'woocommerce' ),
        'type'          => 'text',
        'placeholder'   => '',
        'desc_tip'      => true,
    ),
           ...
           function wcso_review_order_shipping_options() {
            ...
            function add_wcso_shipping_methods( $methods ) {
                $methods['wcso_local_shipping'] = $this; 
                return $methods;
            }
    ...
    <input type="hidden" name="origin_address"  id="origin_address" value="'.$origin_address.'" />
     <input type="hidden" name="origin_street"  id="origin_street" value="'.$origin_street.'" />
      <input type="hidden" name="origin_city"  id="origin_city" value="'.$origin_city.'" />
	   ...
	   <input type="hidden" name="display_frontend" id="display_frontend" value="'.$display_frontend.'" />
...
function post_sendbox_shipment() {
	...
	$display_frontend=sanitize_text_field($_REQUEST['display_frontend']);
	...
	$sendbox_options['display_frontend']=$display_frontend;	 
...
if($response_body->{'fetch_rate_status'}=="failed")
{
	$output=$response_body->{'reason'};
	echo $output;
	  wp_die();
}
...
function load_sendbox_carriers() {
      ...
      function get_sendbox_shipping_quotes_frontend($package)
    {
        ...
        $auth=$auth_header;
        $payload=buildFrontendPayload($package);
...
wp_die();       
   ...
   function buildFrontendPayload($package)
  {
       ...
       $j=0;
         ...
         foreach ( $package['contents'] as $item_id => $values ) {
           ...
           if($values['data']->get_weight()!="" ||$values['data']->get_weight()!=null)
...
$countries_obj   = new WC_Countries();
 ...
 $dest_states=$countries_obj->get_states($package['destination']['country']);
 if($dest_states)
 $destination_state=$dest_states[$package['destination']['state']];
 else
 ...
 $shippingCountryCode =$package['destination']['country'];
 $dest_postal_code = $package[ 'destination' ][ 'postcode' ];
 if($dest_postal_code==null || $dest_postal_code=="")
  $dest_postal_code="";
  ...
  "destination_country_code": "'.    $shippingCountryCode.'",
   "destination_post_code": "'.    $dest_postal_code.'",
  ...
  function build_sendbox_shipment_payload($order,$selected_rate_id,$fee,$options)
  {
       ...
       $j=0;
         ...
         foreach ( $order->get_items() as $item_id => $values ) {
            ...
            if($values->get_product()->get_weight()!="" ||$values->get_product()->get_weight()!=null)
              $weight=$values->get_product()->get_weight()*$values['quantity'];
           else
              $weight=0;
         ...
         $items_string.=']';
          $fee=$order->get_total_shipping()+$fee;
...
$shippingCountry=$countries[$shippingCountryCode];

$dest_states=$countries_obj->get_states($order->get_shipping_country());

if($dest_states)
$destination_state=$dest_states[$order->get_shipping_state()];
 else
 $destination_state=$order->get_shipping_state(); 

 $dest_postal_code = $order->get_shipping_postcode();
  if($dest_postal_code==null || $dest_postal_code=="")
  $dest_postal_code="";
  ...
  "accept_value_on_delivery": '.$accept_value_on_delivery.',
  "amount_to_receive": '.$fee.',
  "fee_payment_channel_code": "cash",
  "channel_code": "website",
...
$countries_obj   = new WC_Countries();
 $states=$countries_obj->get_states($order->get_billing_country());
 $selected_state= $states[$origin_state];
